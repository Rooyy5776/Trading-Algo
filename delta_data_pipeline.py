...
class DeltaHistoricalFetcher:
    ...
    def fetch_range(self, symbol: str, resolution: str, *, days_back: int = DEFAULT_LOOKBACK_DAYS,
        ...
        cursor = start_ts
        if resume:
            # NOTE: this used to require an EXACT match on start_ts/end_ts against the
            # checkpoint before resuming. That's wrong for the realistic case this
            # pipeline actually runs under: default --lookback-days computes end_ts=now
            # fresh on every invocation, so a plain re-run after a crash (or just a
            # later day) never has the exact same window as before, and resume would
            # silently no-op and re-download everything. Fixed: resume from the saved
            # cursor whenever it usefully falls inside [start_ts, end_ts), regardless of
            # whether the window's edges match exactly. This also turns a re-run of an
            # already-*complete* job into a cheap incremental "fetch what's new since
            # last time" instead of a full re-download, since the sliding lookback
            # window's new end_ts is normally later than the old completed cursor.
            state = ckpt.load()
            saved_cursor = state.get("cursor_ts") if state else None
            if saved_cursor is not None and start_ts <= saved_cursor < end_ts:
                cursor = saved_cursor
                logger.info("resuming %s %s from checkpoint cursor %s (job was %s; requested range %s -> %s)",
                            symbol, resolution, iso(cursor),
                            "complete -> fetching only what's new" if state.get("complete") else "incomplete -> continuing",
                            iso(start_ts), iso(end_ts))
            elif saved_cursor is not None and saved_cursor >= end_ts:
                logger.info("%s %s checkpoint cursor %s already covers requested end %s; nothing new to fetch",
                            symbol, resolution, iso(saved_cursor), iso(end_ts))
                cursor = end_ts
            elif saved_cursor is not None:
                logger.info("%s %s checkpoint cursor %s is before requested start %s (window moved past it); "
                            "starting fresh from requested start, old data on disk is kept", symbol, resolution,
                            iso(saved_cursor), iso(start_ts))
...
def main(argv=None):
    ...
    if args.funding:
        # persisted to disk (not just logged) so "what did the funding check find"
        # survives after the terminal/log is gone — lands under funding/, the
        # directory section 10 reserved for exactly this.
        ff = FundingRateFetcher()
        funding_dir = os.path.join(args.data_dir, "funding")
        ensure_dir(funding_dir)
        for sym in symbols:
            result = ff.fetch_history(sym)
            with open(os.path.join(funding_dir, f"{sym}.json"), "w") as f:
                json.dump(result, f, indent=2, default=str)
            logger.info("funding history for %s: %s", sym, result["status"])
    ...
    if args.trades:
        # same reasoning as funding above: persist the outcome (OK or NOT_AVAILABLE)
        # to order_flow/SYMBOL/trades_status.json even when no trades were written,
        # so a failed/soft-failed symbol is visible on disk, not just in the log.
        tf_fetcher = PublicTradesFetcher(data_dir=args.data_dir)
        for sym in symbols:
            result = tf_fetcher.fetch(sym)
            status_dir = os.path.join(args.data_dir, "order_flow", sym)
            ensure_dir(status_dir)
            with open(os.path.join(status_dir, "trades_status.json"), "w") as f:
                json.dump(result, f, indent=2, default=str)
            logger.info("trades for %s: %s", sym, result["status"])
